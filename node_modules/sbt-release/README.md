...
```bash
npm install release-it-snapshot -g
```
...
```bash
release-it-snapshot minor
release-it-snapshot 0.8.3
release-it-snapshot 2.0.0-rc.3
```

Create a pre-release using `prelease`, `prepatch`, `preminor`, or `premajor`:

```bash
release-it-snapshot premajor --prereleaseId="beta"
release-it-snapshot premajor
```
