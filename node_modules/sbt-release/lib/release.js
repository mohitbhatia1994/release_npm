...
function gitAdd() {
    return shelljs.exec('git add .');
}

function gitCommit(commitMessage) {
    return shelljs.exec(`'git commit -m "${commitMessage}"'`);
}

function gitPush() {
    return shelljs.exec('git push');
}

function gitTag(tag) {
    return shelljs.exec(`'git push v${tag}'`);
}

function gitPushTag() {
    return shelljs.exec('git push origin --tags');
}
...
function afterRelease() {
    ...
    // const nextVersion = IncVersion(version, incrementType);
